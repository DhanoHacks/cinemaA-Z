...
def save(response):
    ...
    for page in pages:
        ...
        for container in movie_div:
            ...
            sleep(randint(3,10))
            rotten_tomato_url = "https://www.rottentomatoes.com/m/" + str(re.sub("[^a-z^_]","",name.lower().replace("-"," ").replace(" ","_"))) + "_" + str(year)
            ...
            if len(soup1.find_all('div',class_="meta-value"))<3:
                sleep(randint(3,10))
                rotten_tomato_url = "https://www.rottentomatoes.com/m/" + str(re.sub("[^a-z^_]","",name.lower().replace("-"," ").replace(" ","_")))
                rotten_tomato_urls.append(rotten_tomato_url)
                rotten_tomato_page = requests.Session().get(rotten_tomato_url)
                soup1 = BeautifulSoup(rotten_tomato_page.text, 'html.parser')
            ...
            sleep(randint(3,5))
            metacritic_url = "https://www.metacritic.com/movie/" + re.sub("[^a-z^-]","",str(name.lower().replace("-"," ").replace(" ","-")))
...
def search_results_view(response):
    ...
    if len(movie)>0:
        ...
        with requests.Session() as s:
            ...
            for container in movie_div[0:1]:
                #print(container)
                # Scraping the movie's name
                name = container.h3.a.text
                ...
                if len(soup1.find_all('div',class_="meta-value"))<3:
                    ...
                    for elem in similar:
                        similar_for_movie.append(elem.text.replace('"',""))
                    if len(soup1.find_all('div',class_="meta-value"))<3:
                        return HttpResponse(soup1.text+" "+rotten_tomato_url+" "+name)
                        continue
